...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  // Get repo from the opportunity's saved github_repos (set during Investigate flow)
  const savedRepos = Array.isArray(opportunity.github_repos) ? opportunity.github_repos as string[] : []
  const githubRepo = savedRepos[0]?.trim() || null
